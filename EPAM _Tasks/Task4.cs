...
public class Program
{
	public static bool FindMinMax(int[][] array, out int min, out int max)
	{
		...
		min = 0;
		max = 0;
		GetInitialMinMax(array, ref min, ref max, ref containFlag);
	...
	public static void GetInitialMinMax(int[][] array, ref int Min, ref int Max, ref bool containFlag)
	{
		for (int i = 0; i < array.Length; i++)
		{
			if (array[i] == null)
			...
			for (int j = 0; j < array[i].Length ; j++)
			{
				if (array[i][j].GetType() == typeof(int))
				{
					Min = array[i][j];
					Max = array[i][j];
					containFlag = true;
